...
public class TestContainerInfo
{
    ...
    public TestContainerInfo(Type containerType)
    {
        ...
        TestContextProperty = containerType.GetProperty("TestContext", BindingFlags.Static | BindingFlags.Public);
        var assignContext = CreateWrappedDelegate<AssignTestContext>(TestContextProperty);
    ...
    private TDelegate CreateWrappedDelegate<TDelegate>(PropertyInfo property) where TDelegate : Delegate
    {
        if (property == null)
        {
            return null;
        ...
        {
            return property.SetMethod.CreateDelegate<TDelegate>(null);
        }
        else
        {
            return property.SetMethod.CreateDelegate<TDelegate>(Target);
        }
    }
}
